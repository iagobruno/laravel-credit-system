...
var amountHiddenInput = document.querySelector('input[name="amount"]');
...
updateHiddenAmountInput();
var hasCardInputs = document.getElementsByClassName('card-inputs').length > 0;
var paymentErrorOutput = document.getElementById('card-errors');
var cardHolderName = document.getElementById('card-holder-name-input');
var button = form.querySelector('button[type="submit"]:not(disabled)');
var keys = JSON.parse(document.getElementById('stripe-keys').innerText);
var stripe = Stripe(keys.stripe_pub_key);
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _context.next = 4;
            return stripe.confirmCardSetup(keys.setup_secret, {
              payment_method: {
                card: card,
                billing_details: {
          ...
          case 4:
            ...
            return _context.abrupt("return", showCardError("Payment failed: ".concat(error.message)));
...
function showCardError(msg) {
  ...
  if (msg) button.setAttribute('disabled', 'true');else button.removeAttribute('disabled');
}
})();
